...
```shell
vi /usr/local/mysql/master/conf/my.cnf
```
...
```sql
-- 创建slave用户
CREATE USER 'blueo_slave'@'%';
-- 设置密码
ALTER USER 'blueo_slave'@'%' IDENTIFIED WITH mysql_native_password BY '123456';
-- 授予复制权限
GRANT REPLICATION SLAVE ON *.* TO 'blueo_slave'@'%';
-- 刷新权限
FLUSH PRIVILEGES;
```
...
docker run -d \
-p 3307:3306 \
-v /use/local/mysql/slave1/conf:/etc/mysql/conf.d \
-v /use/local/mysql/slave1/data:/var/lib/mysql \
-e MYSQL_ROOT_PASSWORD=123456 \
--name mysql-slave1 \
mysql:8.0.29;
```
...
```shell
vi /use/local/mysql/slave1/conf/my.cnf
```
...
```sql
CHANGE MASTER TO MASTER_HOST='192.168.43.100', 
MASTER_USER='blueo_slave',MASTER_PASSWORD='123456', MASTER_PORT=3306,
MASTER_LOG_FILE='binlog.000006',MASTER_LOG_POS=3114; 
```

https://www.jianshu.com/p/f0e4de651388
...
```java
package com.buleo.shardingjdbcdemo.entity;
...
```java
package com.buleo.shardingjdbcdemo.mapper;
...
```java
package com.buleo.shardingjdbcdemo;
...
```java
package com.buleo.shardingjdbcdemo.controller;
...
docker run -d \
-p 3301:3306 \
-v /buleo/server/user/conf:/etc/mysql/conf.d \
-v /buleo/server/user/data:/var/lib/mysql \
-e MYSQL_ROOT_PASSWORD=123456 \
--name server-user \
mysql:8.0.29
```
...
docker run -d \
-p 3302:3306 \
-v /buleo/server/order/conf:/etc/mysql/conf.d \
-v /buleo/server/order/data:/var/lib/mysql \
-e MYSQL_ROOT_PASSWORD=123456 \
--name server-order \
mysql:8.0.29
```
...
docker run -d \
-p 3310:3306 \
-v /buleo/server/order0/conf:/etc/mysql/conf.d \
-v /buleo/server/order0/data:/var/lib/mysql \
-e MYSQL_ROOT_PASSWORD=123456 \
--name server-order0 \
mysql:8.0.29
```
...
docker run -d \
-p 3311:3306 \
-v /buleo/server/order1/conf:/etc/mysql/conf.d \
-v /buleo/server/order1/data:/var/lib/mysql \
-e MYSQL_ROOT_PASSWORD=123456 \
--name server-order1 \
mysql:8.0.29
```
...
public void testHash(){
    ...
    System.out.println("buleo001".hashCode() % 2);
    System.out.println("buleo0011".hashCode() % 2);
}
```
...
```java
package com.buleo.shargingjdbcdemo.mapper;
...
```shell
docker run -d \
-v /buleo/server/proxy-a/conf:/opt/shardingsphere-proxy/conf \
-v /buleo/server/proxy-a/ext-lib:/opt/shardingsphere-proxy/ext-lib \
...
将MySQl驱动上传至`/blueo/server/proxy-a/ext-lib`目录
...
将配置文件上传至`/blueo/server/proxy-a/conf`目录
...
```java
package com.buleo.shardingproxydemo.entity;
...
```java
package com.buleo.shardingproxydemo.mapper;
...
```java
package com.buleo.shardingproxydemo;
